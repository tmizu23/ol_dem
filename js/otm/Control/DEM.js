...
otm.Control = new Object();
otm.Control.DEM = OpenLayers.Class(OpenLayers.Control, {
    ...
    handleEvent: function (evt) {
        ...
        if (layers.length > 0) {
            ...
            for (var i = 0, len = layers.length; i < len; i++) {
                ...
                infoLookup[idx] = this.getFeatureInfo(layer, lonLat);
            }
            this.callback(infoLookup, lonLat, evt.xy);
        }
    },
    getFeatureInfo: function (layer, lonlat) {
        var tileInfo = layer.getTileData(lonlat);
        if (tileInfo && tileInfo.tile) {
            return tileInfo.tile.getFeatureInfo(tileInfo.i, tileInfo.j);
        }
        return null;
    },
    /**
    ...
    findLayers: function () {
        ...
        for (var i = candidates.length - 1; i >= 0; --i) {
            layer = candidates[i];
            if (typeof layer.tileClass.prototype.getFeatureInfo == 'function') {
                layers.push(layer);
            }
        }
        return layers;
